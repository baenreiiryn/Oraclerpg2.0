...
function loadClassNormalizer(){if(document.querySelector('script[data-homebrew-class-normalizer]'))return;const s=document.createElement('script');s.src='/homebrew-class-normalizer.js';s.dataset.homebrewClassNormalizer='';s.defer=true;document.head.appendChild(s)}
...
const obs=new MutationObserver(clean);obs.observe(document.documentElement,{childList:true,subtree:true});window.addEventListener('oraclerpg:campaigndraft',()=>{clearFree();setTimeout(clean,0)});loadClassNormalizer();clearFree();clean();
})();
