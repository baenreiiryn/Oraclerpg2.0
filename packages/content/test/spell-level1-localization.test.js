...
const catalog = JSON.parse(fs.readFileSync(path.join(here, "../locales/pt-BR/srd-5.2/spells-level-1.json"), "utf8"));
const spells = canonical.items.filter((spell) => spell.data?.level === 1);

test("PT-BR level-1 catalog exactly covers every canonical level-1 spell", () => {
  ...
  assert.equal(spells.length, 57);
  assert.equal(localizedIds.length, spells.length);
  assert.deepEqual(missing, []);
  assert.deepEqual(unexpected, []);
});

test("level-1 localization contains only presentation paths", () => {
...
test("level-1 localization preserves canonical identity and mechanics", () => {
