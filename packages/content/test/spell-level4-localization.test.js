...
const catalog = JSON.parse(fs.readFileSync(path.join(here, "../locales/pt-BR/srd-5.2/spells-level-4.json"), "utf8"));
const spells = canonical.items.filter((spell) => spell.data?.level === 4);

test("PT-BR level-4 catalog exactly covers every canonical level-4 spell", () => {
  ...
  assert.equal(spells.length, 34);
  assert.equal(localizedIds.length, spells.length);
  assert.deepEqual(missing, []);
  assert.deepEqual(unexpected, []);
});

test("level-4 localization contains only presentation paths", () => {
...
test("level-4 localization preserves canonical identity and mechanics", () => {
