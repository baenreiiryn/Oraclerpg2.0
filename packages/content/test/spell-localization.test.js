...
const catalog = JSON.parse(fs.readFileSync(path.join(here, "../locales/pt-BR/srd-5.2/spells-cantrips.json"), "utf8"));
const cantrips = canonical.items.filter((spell) => spell.data?.level === 0);

test("PT-BR cantrip catalog exactly covers every canonical level-0 spell", () => {
  const canonicalIds = new Set(cantrips.map((spell) => spell.canonicalId));
  const localizedIds = Object.keys(catalog.entries);
  const missing = cantrips.filter((spell) => !catalog.entries[spell.canonicalId]).map((spell) => spell.canonicalId);
  const unexpected = localizedIds.filter((canonicalId) => !canonicalIds.has(canonicalId));

  assert.equal(cantrips.length, 27);
  assert.equal(localizedIds.length, cantrips.length);
  assert.deepEqual(missing, []);
  assert.deepEqual(unexpected, []);
});

test("cantrip localization contains only presentation paths", () => {
...
test("cantrip localization preserves canonical identity and mechanics", () => {
