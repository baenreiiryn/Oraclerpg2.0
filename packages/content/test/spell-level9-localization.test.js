...
const catalog = JSON.parse(fs.readFileSync(path.join(here, "../locales/pt-BR/srd-5.2/spells-level-9.json"), "utf8"));
const spells = canonical.items.filter((spell) => spell.data?.level === 9);

test("PT-BR level-9 catalog exactly covers every canonical level-9 spell", () => {
  ...
  assert.equal(spells.length, 16);
  assert.equal(localizedIds.length, spells.length);
  assert.deepEqual(missing, []);
  assert.deepEqual(unexpected, []);
});

test("level-9 localization contains only presentation paths", () => {
...
test("level-9 localization preserves canonical identity and mechanics", () => {
