...
const catalog = JSON.parse(fs.readFileSync(path.join(here, "../locales/pt-BR/srd-5.2/spells-level-2.json"), "utf8"));
const spells = canonical.items.filter((spell) => spell.data?.level === 2);

test("PT-BR level-2 catalog exactly covers every canonical level-2 spell", () => {
  ...
  assert.equal(spells.length, 58);
  assert.equal(localizedIds.length, spells.length);
  assert.deepEqual(missing, []);
  assert.deepEqual(unexpected, []);
});

test("level-2 localization contains only presentation paths", () => {
...
test("level-2 localization preserves canonical identity and mechanics", () => {
