...
test("inventory SRD 5.2 level-1 spells for PT-BR localization", () => {
  const spells = canonical.items.filter((spell) => spell.data?.level === 1);
  console.log("LEVEL1_INVENTORY_BEGIN");
  console.log(JSON.stringify(spells.map(presentationSnapshot)));
  console.log("LEVEL1_INVENTORY_END");
  console.log(`LEVEL1_COUNT=${spells.length}`);
});
