...
test("inventory SRD 5.2 level-3 spells for PT-BR localization", () => {
  const spells = canonical.items.filter((spell) => spell.data?.level === 3);
  console.log("LEVEL3_INVENTORY_BEGIN");
  console.log(JSON.stringify(spells.map(presentationSnapshot)));
  console.log("LEVEL3_INVENTORY_END");
  console.log(`LEVEL3_COUNT=${spells.length}`);
});
