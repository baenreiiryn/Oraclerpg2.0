...
const catalog = JSON.parse(fs.readFileSync(path.join(here, "../locales/pt-BR/srd-5.2/spells-level-8.json"), "utf8"));
const spells = canonical.items.filter((spell) => spell.data?.level === 8);

test("PT-BR level-8 catalog exactly covers every canonical level-8 spell", () => {
  ...
  assert.equal(spells.length, 17);
  assert.equal(localizedIds.length, spells.length);
  assert.deepEqual(missing, []);
  assert.deepEqual(unexpected, []);
});

test("level-8 localization contains only presentation paths", () => {
...
test("level-8 localization preserves canonical identity and mechanics", () => {
