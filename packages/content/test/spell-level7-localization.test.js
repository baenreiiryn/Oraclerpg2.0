...
const catalog = JSON.parse(fs.readFileSync(path.join(here, "../locales/pt-BR/srd-5.2/spells-level-7.json"), "utf8"));
const spells = canonical.items.filter((spell) => spell.data?.level === 7);

test("PT-BR level-7 catalog exactly covers every canonical level-7 spell", () => {
...
test("level-7 localization contains only presentation paths", () => {
...
test("level-7 localization preserves canonical identity and mechanics", () => {
