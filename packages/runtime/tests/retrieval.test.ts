...
const memory: CampaignMemoryState = {
  ...
  revision: 8,
  records: [
    { memoryId: "public-event", campaignId: "campaign-1", kind: "EPISODIC", summary: "The party found a crimson sigil beneath the fortress.", visibility: "PUBLIC", importance: 0.8, source: { sceneId: "scene-1", entityIds: ["fortress"] }, entityIds: ["fortress"], createdAtMemoryRevision: 5, updatedAtMemoryRevision: 5 },
    { memoryId: "pc1-secret", campaignId: "campaign-1", kind: "DISCOVERY", summary: "The hidden passage opens behind the crimson sigil.", visibility: "ACTOR_ONLY", actorIds: ["pc-1"], importance: 1, source: { worldFactId: "secret-route" }, entityIds: ["fortress"], createdAtMemoryRevision: 8, updatedAtMemoryRevision: 8 },
    { memoryId: "gm-secret", campaignId: "campaign-1", kind: "SEMANTIC", summary: "The duke serves the cult.", semanticKey: "duke:allegiance", visibility: "GM_ONLY", importance: 1, source: { worldFactId: "duke-cult" }, entityIds: ["duke", "cult"], createdAtMemoryRevision: 8, updatedAtMemoryRevision: 8 },
  ],
};
...
const world: CampaignWorldState = {
  ...
  activeScene: { sceneId: "scene-1", locationId: "fortress", participantIds: ["pc-1", "pc-2", "duke"], threatIds: [], openThreads: [], recentEvents: [] },
  actors: [
...
test("hybrid retrieval keeps GM, hidden, and other-actor knowledge out of player results", async () => {
  const result = await engine().retrieve({ campaignId: "campaign-1", actorId: "pc-2", text: "cult duke hidden passage crimson sigil", entityIds: ["duke", "fortress"], maxTokens: 500 });
  assert.equal(result.items.some((item) => item.retrievalId === "memory:gm-secret"), false);
  assert.equal(result.items.some((item) => item.retrievalId === "memory:pc1-secret"), false);
  ...
  assert.ok(result.droppedForVisibility >= 4);
});

test("actor-only memory and fact are retrievable only for the granted actor", async () => {
  const result = await engine().retrieve({ campaignId: "campaign-1", actorId: "pc-1", text: "hidden passage crimson sigil", entityIds: ["fortress"], maxTokens: 500 });
...
test("context budget is a hard ceiling even when high-ranked items do not fit", async () => {
  const result = await engine().retrieve({ campaignId: "campaign-1", actorId: "pc-1", text: "fortress crimson sigil", entityIds: ["fortress"], maxTokens: 12 });
...
test("hybrid ranking combines semantic signals with entity relevance", async () => {
  const external: RetrievalSourcePort = {
    async retrieve() {
      return [
        { retrievalId: "document:weak", source: "DOCUMENT", text: "General history of the fortress.", visibility: "PUBLIC", semanticScore: 0.2, entityIds: ["fortress"], importance: 0.3 },
        { retrievalId: "document:strong", source: "DOCUMENT", text: "Cult symbols and passages beneath the fortress.", visibility: "PUBLIC", semanticScore: 0.95, entityIds: ["fortress", "cult"], importance: 0.9 },
      ];
    },
  };
  const result = await engine([external]).retrieve({ campaignId: "campaign-1", actorId: "pc-1", text: "cult passage fortress", entityIds: ["fortress", "cult"], maxTokens: 500 });
...
test("allowedSources can constrain retrieval to a requested context class", async () => {
  const result = await engine().retrieve({ campaignId: "campaign-1", actorId: "pc-1", text: "fortress", maxTokens: 500, allowedSources: ["WORLD_FACT"] });
