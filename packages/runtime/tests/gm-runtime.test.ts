...
import { OracleGmRuntime, type CompletedGmTurn } from "../src/index.js";
import type {
...
const intent: TurnIntent = { campaignId: "c1", actorId: "pc1", message: "I attack the goblin.", clientRequestId: "req1", sessionId: "s1" };
const state: MechanicalStateSnapshot = { campaignId: "c1", actorId: "pc1", revision: 7, data: {} };
const context: OracleContextPackage = { version: 1, campaignId: "c1", actorId: "pc1", stateRevision: 7, sections: {} };
const manifest: AiCapabilityManifest = { version: 1, capabilities: [{ operation: "rules.request-evaluation", schemaVersion: 1, description: "attack", allowedActorIds: ["pc1"], allowedTargetIds: ["goblin1"], allowedRefs: ["attack:sword"] }] };
const action: StructuredAiActionProposal = { schemaVersion: 1, proposalId: "p1", operation: "rules.request-evaluation", actorId: "pc1", targetIds: ["goblin1"], payload: { actionRef: "attack:sword", requestedResolution: "ATTACK" } };

function buildRuntime(options: { reject?: boolean; outOfCapability?: boolean } = {}) {
  const calls: string[] = [];
  const records: TurnRecord[] = [];
  const proposal: AiTurnProposal = { narrativeDraft: "draft", actions: [options.outOfCapability ? { ...action, targetIds: ["dragon"] } : action] };
  ...
  const operationRouter = {
    async run(request: OracleOperationRequest): Promise<OracleOperationResponse> {
      ...
      if (request.operation === "gm.interpret-turn") {
        return { requestId: request.requestId, operation: request.operation, alias: "oracle-reasoning", output: JSON.stringify(proposal), structuredOutput: proposal, usage: {} };
      }
      return { requestId: request.requestId, operation: request.operation, alias: "oracle-story", output: "Final narration", usage: {} };
    },
  };
  ...
  const runtime = new OracleGmRuntime({
    ...
    retrieval: { async retrieve(query) { calls.push("retrieval"); return { query, items: [], usedTokens: 0, remainingTokens: query.maxTokens, droppedForBudget: 0, droppedForVisibility: 0 }; } },
    operationRouter,
    proposalDecoder: { decode(input) { calls.push("decode"); return input as AiTurnProposal; } },
    actionValidator: { async validate() { calls.push("validate"); return { proposalId: "p1", accepted: !options.reject, ...(options.reject ? { reason: "illegal" } : {}) }; } },
    actionExecutor: { async execute() { calls.push("execute"); return { proposalId: "p1", operation: "rules.request-evaluation", status: "applied", result: { hit: true } }; } },
    memoryExtractor: { async extract() { calls.push("memory-extract"); return [{ candidateId: "m1", kind: "EPISODIC", summary: "A goblin was attacked.", visibility: "PUBLIC", importance: 0.5, source: {} }]; } },
...
test("AI-9/10 executes a complete authoritative GM turn through semantic operations", async () => {
  ...
  assert.ok(calls.indexOf("execute") < calls.indexOf("operation:gm.narrate"));
  assert.ok(calls.indexOf("persist") < calls.indexOf("memory-extract"));
  assert.ok(calls.includes("operation:gm.interpret-turn"));
...
test("misaligned context aborts before any routed AI operation", async () => {
