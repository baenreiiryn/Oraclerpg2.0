...
const stillUnsupported=[];for(const issue of report.schemaUnsupported??[]){if(issue.key==="img")report.providerOnlyIgnored.push({key:"img",value:issue.value,reason:"Effect icon mutation is Foundry presentation metadata."});else stillUnsupported.push(issue);}
report.schemaUnsupported=stillUnsupported;report.schemaUnsupportedCount=stillUnsupported.length;report.runtimeStatusesCanonicalized=canonicalized;report.manualSemantics=[];report.manualSemanticCount=0;
await fs.writeFile(FILE,JSON.stringify(comp,null,2)+"\n");await fs.writeFile(REPORT,JSON.stringify(report,null,2)+"\n");
console.log(JSON.stringify({schemaUnsupportedCount:report.schemaUnsupportedCount,runtimeStatusesCanonicalized:canonicalized,providerOnlyIgnoredCount:report.providerOnlyIgnored.length},null,2));if(stillUnsupported.length)process.exitCode=1;