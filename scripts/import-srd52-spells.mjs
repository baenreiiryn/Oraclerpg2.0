...
const HANDLED_FIELDS = new Set([
  ...
  "scalingLevelDice","abilityCheck","damageResist","conditionImmune","damageImmune","damageVulnerable","srd52","basicRules2024"
]);
...
function rangeOf(record) {
  const r = record.range ?? { type:"point", distance:{type:"special"} };
  return { type:r.type, distance:{ type:r.distance?.type ?? "special", ...(r.distance?.amount != null ? {amount:Number(r.distance.amount)}:{}) } };
}
...
function classLists(record, sources) {
  const entries = sources?.[record.source]?.[record.name]?.class ?? [];
...
function baseActivity(record) {
  ...
  if (range.distance.type === "feet" || range.distance.type === "miles" || ["touch","sight","unlimited","self"].includes(range.distance.type)) {
  ...
  if (["cone","cube","emanation","line","sphere"].includes(range.type)) {
    activity.target = { type:"point", area:{ shape:range.type, ...(range.distance.amount != null ? {size:{value:range.distance.amount, unit:range.distance.type === "miles" ? "mile" : "ft"}}:{}) } };
  ...
  if (summon || attacks.length > 1 || saves.length > 1 || checks.length > 1 || record.damageInflict?.length || record.conditionInflict?.length) {
    activity.manualAdjudication = { required:true, reason:"5etools exposes discovery metadata for this spell but does not fully encode every executable formula, target branch, repeat action, or outcome. The canonical rules text is preserved for semantic enrichment.", fallback:"promptGM" };
  }
  return activity;
}
function mapRecord(record, sources) {
  const ds = durations(record);
  ...
  return {
    level:Number(record.level), school:SCHOOL[record.school], ritual:Boolean(record.meta?.ritual),
    concentration:ds.some(d=>d.concentration), castingTimes:castingTimes(record), range:rangeOf(record), durations:ds,
    components:components(record), activities:[baseActivity(record)], ...(scale ? {scaling:scale}:{}), ...(lists.length ? {spellLists:lists}:{}),
...
const candidates = (spellJson.spell ?? []).filter(spell => spell.srd52 === true);
const records = [];
const diagnostics = [];
for (const source of candidates) {
  const extra = Object.keys(source).filter(key => !HANDLED_FIELDS.has(key));
  if (extra.length) diagnostics.push({name:source.name,status:"unmapped-source-fields",fields:extra});
  try {
    const data = mapRecord(source, sources);
    records.push({
      id:spellId(source.name), canonicalId:spellId(source.name), entityType:"spell", name:source.name,
      ...
      provenance:{origin:"import",provider:"5etools",sourceKey:`${source.name}|${source.source}`,importedAt:new Date().toISOString(),adapterVersion:"0.1.0",mapperVersion:"0.1.0"},
      schemaVersion:1,data,
      relations:(data.spellLists??[]).map(ref=>({type:"availableTo",targetCanonicalId:ref.canonicalId})),
      metadata:{tags:["srd-5.2","5etools","spell",`level-${source.level}`]}
    });
  } catch(error) { diagnostics.push({name:source.name,status:"mapper-error",error:error instanceof Error?error.message:String(error)}); }
}
...
await fs.writeFile(path.join(OUT_DIR,"spells-import-report.json"),JSON.stringify({generatedAt:new Date().toISOString(),upstream:[URL_SPELLS,URL_SOURCES],sourceCandidates:candidates.length,imported:records.length,diagnostics},null,2)+"\n");
