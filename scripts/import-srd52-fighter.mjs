...
case 'Fighting Style':
 ...
 d.classRules={selectionPolicies:[{collectionId:'fighter-fighting-style',repeatable:false,uniqueBy:'entity',replacement:{trigger:trigger('custom','Whenever you gain a Fighter level, you may replace the selected Fighting Style feat.'),count:constant(1)}}]};break;
case 'Second Wind':
 d.grants=[{type:'resource',value:{id:'second-wind',max:runtime('class.fighter.secondWindUses'),recovery:[{period:'shortRest',amount:constant(1)},{period:'longRest',amount:'all'}]}}];
 d.activities=[{id:'second-wind-healing',name:'Second Wind',kind:'healing',activation:{type:'bonusAction'},target:{type:'self'},healing:[{formula:'1d10 + @classes.fighter.level',type:'healing'}],uses:{max:constant(1),recovery:[],sharedResourceId:'second-wind'},description:'Regain 1d10 + Fighter level Hit Points.'}];break;
case 'Weapon Mastery':
 d.classRules={entityCollections:[{id:'fighter-weapon-masteries',entityTypes:['item'],capacity:runtime('class.fighter.weaponMasteryCount'),filter:pred('Simple or Martial weapon with a mastery property.'),chooseOn:trigger('onApply','Choose mastered weapon kinds.'),replace:{trigger:trigger('onRest','After a Long Rest, one mastered weapon choice may be replaced.'),count:constant(1),filter:pred('Simple or Martial weapon with a mastery property.')},progression:[{level:1,capacity:constant(3)},{level:4,capacity:constant(4)},{level:10,capacity:constant(5)},{level:16,capacity:constant(6)}]}]};
 d.properties=['uses-selected-weapon-mastery-properties'];break;
case 'Action Surge':
 d.grants=[{type:'resource',value:{id:'action-surge',max:runtime('class.fighter.actionSurgeUses'),recovery:[{period:'shortRest',amount:'all'},{period:'longRest',amount:'all'}]}}];
 d.classMechanics={generatedActions:[{trigger:trigger('onActivate','On your turn, expend Action Surge.'),action:'custom',count:constant(1),predicate:pred('Grant one additional action that cannot be used for the Magic action; Action Surge can be used only once on a turn.')} ]};
 d.properties=['extra-action:not-magic','once-per-turn'];break;
case 'Tactical Mind':
 d.rollRules=[{id:'tactical-mind',target:'abilityCheck',operation:'add',trigger:trigger('onFailedCheck','After failing an ability check, expend one Second Wind use and add 1d10.'),formula:'1d10',description:'If the modified check still fails, the Second Wind use is not expended.'}];
 d.resourcePreservationRules=[{id:'tactical-mind-refund',resource:'classResource',trigger:trigger('onFailedCheck','After Tactical Mind resolves.'),predicate:pred('The ability check still fails after adding the Tactical Mind d10.'),preserveOnSuccess:true,description:'Retain the Second Wind use if the check still fails.'}];
 d.properties=['uses-resource:second-wind'];break;
case 'Extra Attack':d.properties=['attack-action-count:2'];break;
case 'Tactical Shift':
 d.classRules={movementPermissions:[{subject:'self',permissions:['ignoreOpportunityAttacks'],predicate:pred('Immediately after activating Second Wind as a Bonus Action; move up to half Speed.'),duration:{endTriggers:[trigger('custom','End after the granted half-Speed movement.')]}}]};
 d.movementInteractionRules=[{id:'tactical-shift-move',action:'move',subject:'self',distance:formula('floor(@attributes.movement.walk / 2)'),trigger:trigger('custom','Immediately after activating Second Wind as a Bonus Action.'),predicate:pred('This movement does not provoke Opportunity Attacks.')}];break;
case 'Indomitable':
 d.grants=[{type:'resource',value:{id:'indomitable',max:runtime('class.fighter.indomitableUses'),recovery:[{period:'longRest',amount:'all'}]}}];
 d.rollRules=[{id:'indomitable-reroll',target:'savingThrow',operation:'reroll',trigger:trigger('onFailedSave','Expend one use of Indomitable after failing a saving throw.'),formula:'@classes.fighter.level',description:'Reroll the failed save, add Fighter level to the new roll, and use the new roll.'}];break;
case 'Tactical Master':
 d.classMechanics={runtimeChoices:[{id:'fighter-tactical-mastery-attack',options:['Push','Sap','Slow'],chooseOn:trigger('onAttack','When attacking with a weapon whose mastery property you can use.'),activeUntil:trigger('custom','Ends after this attack resolves.')} ]};
 d.properties=['replace-current-mastery-for-attack'];break;
case 'Two Extra Attacks':d.properties=['attack-action-count:3'];break;
case 'Studied Attacks':
 d.effects=[{id:'studied-attacks-advantage',trigger:trigger('onMiss','When you miss a creature with an attack roll.'),duration:{type:'untilTrigger',endTrigger:trigger('custom','Ends after your next attack against that creature or at the end of your next turn.')},modifiers:[{target:{domain:'attackRoll'},mode:'advantage',predicate:pred('Only the next attack roll against the same creature before the end of your next turn.')}]}];break;
...
case 'Improved Critical':
 d.classMechanics={resolutionOverrides:[{domain:'critical',mode:'custom',value:constant(19),predicate:pred('Weapon attacks and Unarmed Strikes score a Critical Hit on a natural 19 or 20.')} ]};d.properties=['critical-threshold:19'];break;
case 'Remarkable Athlete':
 d.modifiers=[{target:{domain:'initiative'},mode:'advantage'},{target:{domain:'skillCheck',skill:'Athletics'},mode:'advantage'}];
 d.classRules={movementPermissions:[{subject:'self',permissions:['ignoreOpportunityAttacks'],predicate:pred('Immediately after scoring a Critical Hit; move up to half Speed.'),duration:{endTriggers:[trigger('custom','End after the granted movement.')]}}]};
 d.movementInteractionRules=[{id:'remarkable-athlete-critical-move',action:'move',subject:'self',distance:formula('floor(@attributes.movement.walk / 2)'),trigger:trigger('onCritical','Immediately after scoring a Critical Hit.'),predicate:pred('This movement does not provoke Opportunity Attacks.')}];break;
case 'Additional Fighting Style':d.grants=[{type:'entity',choice:featChoice('fightingStyle')}];break;
case 'Heroic Warrior':d.triggeredGrants=[{id:'heroic-warrior-inspiration',trigger:trigger('onTurnStart','During combat, if you start your turn without Heroic Inspiration.'),grant:'heroicInspiration',predicate:pred('Only during combat and only if the Fighter lacks Heroic Inspiration.')}];break;
case 'Superior Critical':
 d.classMechanics={resolutionOverrides:[{domain:'critical',mode:'custom',value:constant(18),predicate:pred('Weapon attacks and Unarmed Strikes score a Critical Hit on a natural 18-20.')} ]};d.properties=['critical-threshold:18'];break;
case 'Survivor':
 d.rollRules=[{id:'survivor-death-save-advantage',target:'savingThrow',operation:'advantage',trigger:trigger('onSave','When making a Death Saving Throw.'),predicate:pred('Death Saving Throw only.')}];
 d.classMechanics={resolutionOverrides:[{domain:'save',mode:'replaceTotal',value:constant(20),predicate:pred('On a Death Saving Throw natural roll of 18-20, gain the benefit of rolling a 20.')} ]};
 d.triggeredGrants=[{id:'champion-heroic-rally',trigger:trigger('onTurnStart','If Bloodied and at least 1 Hit Point, regain Hit Points.'),grant:'hitPoints',value:formula('5 + @abilities.con.mod'),predicate:pred('The Champion is Bloodied and has at least 1 Hit Point.')}];break;
}return d;}
...
const champion={...base('subclass','Champion',SUBCLASS_ID,'XPHB:subclass:Fighter:Champion'),data:{parentClass:entityRef('class','Fighter',CLASS_ID),advancement:[3,7,10,15,18].map(level=>({level,grants:subByLevel.get(level)??[]})),text:rules(sub.entries??[])}};
